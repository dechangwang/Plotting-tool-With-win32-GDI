	...
	case WM_CREATE:
		{
			...
			hButton1 = CreateWindow(L"BUTTON", L"Plot", WS_CHILD|WS_VISIBLE, locationX-50, 250, 200, 30, hwnd, (HMENU)PLOTING, hInst, 0);
			hbgColor = CreateWindow(L"BUTTON", L"bColor", WS_CHILD|WS_VISIBLE, locationX-50, 300, 200, 30, hwnd, (HMENU)CHANGE_BGCOLOR, hInst, 0);
			...
			func1 = CreateWindow(L"EDIT", L"",  WS_CHILD | WS_VISIBLE | WS_BORDER, 800, 100, 300, 30, hwnd, (HMENU)11, hInst, 0);
			func2 = CreateWindow(L"EDIT", L"",  WS_CHILD | WS_VISIBLE | WS_BORDER, 800, 150, 300, 30, hwnd, (HMENU)12, hInst, 0);
			func3 = CreateWindow(L"EDIT", L"",  WS_CHILD | WS_VISIBLE | WS_BORDER, 800, 200, 300, 30, hwnd, (HMENU)13, hInst, 0);
		    
			//TextOut(hdc,points[0].x,points[0].y,L"Hello",4);
		}break;
   case WM_LBUTTONUP:
   case WM_RBUTTONUP:
		{
    ...
    case WM_PAINT:  
		...
		if(points[0].x<points[1].x)
			for(int i=1; i<=north*10; i++)
			{
				...
				TextOut(hdc, points[0].x+5 + moveRealX, points[0].y-i*realSpace-3 + moveRealY, str,number+2);
			...
			for(int i = 1;i<=east*10;i++)
			{
				...
				_stprintf_s(str,_T("%.3f"),((i+movePartY)*space));
				TextOut(hdc,points[0].x+i*realSpace+6,points[0].y+5,str,number+2);
				}
			...
			for(int i = 1;i<=south*10;i++)
			{
				...
				_stprintf_s(str,_T("%.3f"),(-(i+movePartY)*space));
				TextOut(hdc, points[0].x+5, points[0].y+i*realSpace-3,str,number+3);
			}
			//绘制X轴负半轴上的刻度
			//if(points[0].x<points[1].x)
			for(int i = 1;i<=west*10;i++)
			{
				...
				if(points[0].x-i*realSpace < points[1].x){
					...
					int number = (i+movePartY)*space<10?1:((i+movePartY)*space<100?2:3);
					_stprintf_s(str,_T("%.3f"),(-(i+movePartY)*space));
					TextOut(hdc, points[0].x-i*realSpace, points[0].y+5,str,number+3);
				}
